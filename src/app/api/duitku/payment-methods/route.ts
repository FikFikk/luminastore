...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      try {
        const data: DuitkuResponse = JSON.parse(responseText);
        ...
        data.paymentFee.forEach((method: DuitkuPaymentMethod) => {
            ...
            if (methodCode.includes('va') || methodCode.includes('bank') || 
                methodCode.includes('bri') || methodCode.includes('bca') || 
                methodCode.includes('bni') || methodCode.includes('mandiri')) {
                groupedMethods.bank_transfer.push(methodData);
            } else if (methodCode.includes('ovo') || methodCode.includes('gopay') || 
                        methodCode.includes('dana') || methodCode.includes('linkaja') ||
            ...
            } else if (methodCode.includes('alfamart') || methodCode.includes('indomaret')) {
        ...
        console.error("[Duitku Route] JSON parse error:", e);
        return NextResponse.json({
    ...
    console.error("[Duitku Route] Unexpected error:", error);
    return NextResponse.json({
